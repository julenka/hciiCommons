...
Background::Background() : dirtyPixels(true) {}
...
void Background::update(ofxKinect &kinect) {
    ...
    for(int y = 0; y < DEPTH_MAP_HEIGHT; y++) {
        for(int x = 0; x < DEPTH_MAP_WIDTH; x++) {
            ...
            if(depth <= 0 ) continue;
            backgroundCount[i]++;
            backgroundSum[i] += depth;
            float mean = backgroundSum[i] / backgroundCount[i];
            backgroundMean[i] = mean;
            // e[(x - xbar)^2)] = e[x^2] - e[x]^2
            backgroundSqSum[i] += (depth * depth);
        }
    }
    dirtyPixels = true;
}
...
bool Background::isBackground(ofxKinect &kinect, int x, int y) {
    ...
    if(depth - backgroundMean[i] > -300) return true;
    return false;
}
