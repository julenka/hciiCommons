...
class contourSimplify{
	...
	public:
		...
		void simplify(vector <ofVec2f> &contourIn, vector <ofVec2f> &contourOut, float tolerance){
			...
			vector<ofVec3f> polyLineIn;
            vector<ofVec3f> polyLineOut;
			
			//first we copy over the data to a 3d point array
			for(int i = 0; i < length; i++){
                ofVec3f toAdd;
                toAdd.x = contourIn[i].x;
                toAdd.y = contourIn[i].y;
                polyLineIn.push_back(toAdd);
			}
			
			int numPoints = poly_simplify(tolerance, &polyLineIn.front(), length, &polyLineOut.front());
			contourOut.clear();
